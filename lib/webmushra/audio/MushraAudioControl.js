...
MushraAudioControl.prototype.linearMix = function(_sig1, _sig2, _balance) {
  var loopChannels = Math.min(_sig1.numberOfChannels, _sig2.numberOfChannels)
  var loopSamples = Math.min(_sig1.length, _sig2.length)

  var buffer = this.audioContext.createBuffer(loopChannels, loopSamples, _sig1.sampleRate);


  //loop thru channels
  for (var i = 0; i < loopChannels; ++i){
    var s1 = _sig1.getChannelData(i);
    var s2 = _sig2.getChannelData(i);

    var dat = new Float32Array(loopSamples)

    //process
    for (var n = 0; n < loopSamples; ++n){
      dat[n] = s1[n] + (_balance * (s2[n] - s1[n]))
    }
...
MushraAudioControl.prototype.sliderMix = function(_sig1, _sig2, _sliderVal) {
  var mixedAudio = new Stimulus("mixed", null);

  mixedAudio.setAudioBuffer(this.linearMix(_sig1, _sig2, _sliderVal))
  this.conditions[2] = mixedAudio; 
};
