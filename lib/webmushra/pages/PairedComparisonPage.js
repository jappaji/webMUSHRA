...
PairedComparisonPage.prototype.render = function (_parent) {
  ...
  // names
  var trNames = $("<tr><td>A</td><td>B</td><td>Mixed</td></tr>");
  ...
  var mixSlider = $("<div class='slidecontainer'>   <input type='range' min='0' max='1' value='0' step = 0.01 class='slider' onchange='" + this.pageManager.getPageVariableName(this) +".mixAudio(this.value)' id='myRange'> </div>");
  tdResponse.append(mixSlider);
...
PairedComparisonPage.prototype.btnCallbackC = function() {
	this.currentItem = "C";
  ...
  if (label == this.pageManager.getLocalizer().getFragment(this.language, 'pauseButton')) {
    ...
    $(".audioControlElement").text(this.pageManager.getLocalizer().getFragment(this.language, 'playButton'));
    if (this.mushraAudioControl.conditions.length < 3) {
      this.mushraAudioControl.playCondition(0);
    }
    else {
...
PairedComparisonPage.prototype.mixAudio = function(sliderVal) {

  this.mushraAudioControl.sliderMix(this.reference.audioBuffer, this.condition.audioBuffer, sliderVal)
